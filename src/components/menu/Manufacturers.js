...
const Manufacturers = (props) => {
  ...
  const loadTableData = () => { // Get Table Data from Server
    setLoadingData(true);
    axios.post(base + '/manufacturers', { userID: userID }).then((documents) => {
  ...
  const manufacturersActions = { // All Actions on Clients Page
    ...
    deleteRow: () => { // Delete Selected Row
      if (currentSelectedRow !== null) {
        ...
        axios.post(base + '/remove', { collectionName: "Manufacturers", deleteDocumentKey: currentSelectedRow.key }).then((deleteResult) => {
    ...
    saveEditRow: (oldDocument, updatedDocument) => {
      axios.post(base + '/edit', { collectionName: "Manufacturers", queryKey: oldDocument.key, updatedDocument: updatedDocument }).then((editResult) => {
        if(editResult.data){
          loadTableData();
        }
        else{
    ...
    saveCreateRow: (newDocument) => {
      axios.post(base + '/add', { collectionName: "Manufacturers", newDocument: newDocument }).then((insertResult) => {
  ...
  return (
    <ManufacturersContainer>
      {showEditOrNewRow ? <EditRow actions={manufacturersActions} ></EditRow> :
        <div>
          <TableSection>
            <StyledTitle type={"danger"}>יצרנים</StyledTitle>
          ...
          <ButtonsSection>
            <StyledButton type="primary" danger onClick={() => { manufacturersActions.backToMenuPage() }}>חזרה לתפריט</StyledButton>
            <StyledButton type="primary" onClick={() => { manufacturersActions.createRow() }}>חדש</StyledButton>
            <StyledButton type="primary" danger onClick={() => { manufacturersActions.deleteRow() }}>מחק</StyledButton>
            <StyledButton type="primary" onClick={() => { manufacturersActions.editRow() }}>ערוך</StyledButton>
          </ButtonsSection>
        </div>
      }
    </ManufacturersContainer>
